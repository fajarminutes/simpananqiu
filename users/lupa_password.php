...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (mysqli_num_rows($result) > 0) {
         // Username dan Email ditemukan, lanjutkan dengan proses reset password
        $_SESSION['user_verifikasi'] = $pengguna['id_user'];
             // Generate dan simpan kode verifikasi 6 digit ke dalam session
        $verificationCode = rand(100000, 999999);
        $_SESSION['verification_code_vr'] = $verificationCode;

        // Simpan juga data username dan email pengguna untuk validasi
        $username = $pengguna['username'];
        $_SESSION['verification_user_vr'] = $username;
        $email = $pengguna['email'];
        $_SESSION['verification_email_vr'] = $email;
...
$ambil = mysqli_fetch_array($query);
        ...
        $_SESSION['error_message'] = 'Username/Email Salah!.';
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h4>Lupa Password Verifikasi 2 Langkah</h4>
                    </div>
                    <div class="card-body">
                        <p class="text-muted mb-4">Masukkan alamat <b>username dan email Anda</b> untuk mereset password verifikasi 2 langkah.</p>
